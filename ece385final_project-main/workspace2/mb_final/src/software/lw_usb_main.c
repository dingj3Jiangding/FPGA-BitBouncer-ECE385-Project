...
// Player state
int16_t player_x = 0;  // A bit offset from the very left edge
...
void timer_int_handler(void *CallbackRef, u8 TmrCtrNumber) {
    ...
    float speed_multiplier = get_current_speed_multiplier(game_get_time_ms());
    int current_move_speed = (int)(MOVE_SPEED * speed_multiplier);

    // Automatic rightward movement
    int potential_next_x = player_x;
    if (!jumping) { // Or apply horizontal movement during jump if desired
        if (player_x < SCREEN_WIDTH - DINO_WIDTH - current_move_speed) {
            potential_next_x = player_x + current_move_speed;
        } else {
            ...
            reset_counter++;
			set_random_seed(reset_counter * 12345 + game_get_time_ms);
            ...
            // Generate buffs for the new round/scene
			get_buff_spawn_obstacle_status(buff_obstacle_status);
			generate_buffs(buff_obstacle_status, (BRAM_t*)hdmi_ctrl);
			xil_printf("Buffs generated for new round.\n");
        ...
        if (player_x < SCREEN_WIDTH - DINO_WIDTH - MOVE_SPEED) {
            ...
            reset_counter++;
            set_random_seed(reset_counter * 67890 + game_get_time_ms());
			...
			get_buff_spawn_obstacle_status(buff_obstacle_status);
			generate_buffs(buff_obstacle_status, (BRAM_t*)hdmi_ctrl);
			xil_printf("Buffs generated for new round (after jump completion at edge).\n");
...
int main() {
    ...
    srand(12345); // Initial seed for stdlib rand, if buff.c uses it.
    // set_random_seed in map.c controls a custom RNG, also seeded at reset.
    xil_printf("Dinosaur Game with Buffs - %d Rounds to Win\n", VICTORY_ROUNDS);
	...
	get_buff_spawn_obstacle_status(buff_obstacle_status);
	generate_buffs(buff_obstacle_status, (BRAM_t*)hdmi_ctrl);
	xil_printf("Initial buffs generated for the first round.\n");
    ...
    while (1) {
        ...
        if (update_flag && !game_over && !game_victory) {  // Only update animations if game is not over or won
            ...
            if (check_player_collect_buff(player_x, (BRAM_t*)hdmi_ctrl)) {
				player_score += 10;

				// Print buff type collected
				const char* buff_names[] = {"None", "Speed Up", "Speed Down", "Alternating Speed"};
				xil_printf("Buff collected: %s! Score: %d\n",
						   buff_names[player_buff_state.active_buff],
						   player_score);
			}
			// End Buff Collection

			// === MODIFIED: Update buff state to handle duration and alternating effects ===
			update_buff_state(game_get_time_ms());
			...
			draw_all_existing_obstacles((BRAM_t*)hdmi_ctrl);
			update_flag = 0;
		}
	}
