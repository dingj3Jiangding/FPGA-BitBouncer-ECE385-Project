 ...
 * This function resets all animations to the initial position on the left side of the screen
 */
void reset_player_to_start_position(struct Player_animation* run_animation_right,
    ...
    // Update all animation positions to the start position
    update_all_animation_positions(run_animation_right, start_x, GROUND_LEVEL - DINO_HEIGHT, 0);
    update_all_animation_positions(run_animation_left, start_x, GROUND_LEVEL - DINO_HEIGHT, 1);

    // Reset jump animations properly to maintain correct Y positions
    reset_jump_animation(jump_animation_right, start_x);
    reset_jump_animation(jump_animation_left, start_x);
}
